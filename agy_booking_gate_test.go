...
func TestARecordingNobodyMadeIsRefused(t *testing.T) {
	...
	for _, s := range []string{
		...
		// 2026-09-22 conv of يزن: a note to a branch that nobody received.
		"ولا يهمك، أنا رفعت ملاحظة للفريق بفرع إربد عشان يراجعوا الدكتور أسامة ويتواصلوا معك",
		// 2026-09-24: a retouch request "passed to the department".
		"حولت طلبك بالموعد اللي حددتيه (يوم الأربعاء 30/9 الساعة 1:00 الظهر) للقسم المختص بالعيادة",
	...
	for _, s := range []string{
		...
		// False positives from the 2,136-reply scan (2026-09-21..24).
		"تمام سجّلت عندي إنك بتفضلي فرع إربد. دوامنا بالفرع يومياً من الساعة 8:00 صباحاً",
		"يا ريت تزوديني بالتفاصيل (المبلغ، اسم الشخص اللي حوّل، وتقريباً بأي وقت تم التحويل) عشان أقدر أتحقق منها",
		"شو الاسم اللي تم التحويل منه بالضبط",
		"ما في أي تفاصيل مسجلة بخصوص المستشفى، لأن التوصية كانت شفهية وما تسجلت عندي بالسيستم",
	} {
		if agyUnbackedRecordClaim(gateOn(), in, textReply(s)) {
